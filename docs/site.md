...
- The share card is a 3:4 canvas (900 × 1200). Top: title, headline, the chosen name. Middle: the winning dish sprite large with the fly on it (proboscis-out frame); the other dishes in a row below, smaller, greyscale at 40% with a thin line through the plate (drawn per pixel, so it does not depend on canvas filters); ties show every tied sprite in colour side by side with the fly hovering above; in "Do the opposite" the fly's own pick keeps its colour and the fly. Under the sprites the MN9 bar chart carries the numbers (shared scale, at most four rows, then "+N"). Bottom: the four fixed lines and the honesty line on the left, and on the right a QR code (error correction M, 4-module quiet zone) that opens the same comparison, with the short URL in monospace under it for people who cannot scan. "Save image" downloads the PNG. The water honesty line stays in the page footer.
