...
Connectivity only (`scripts/tonic_ffi_path.py`, no simulation): excitatory neurons at most two excitatory hops downstream of the frozen sugar GRN set that are presynaptic to CB0465 `720575940636809646`, ranked by synapses onto CB0465. Sign from `Excitatory x Connectivity`, counts from `Connectivity`; for a two-hop entry the intermediate shown is the one with the strongest sugar → intermediate → neuron chain (max of the smaller of the two counts). Annotation from Schlegel et al. 2024, Shiu names from the SEZ dictionary. Caveat: two excitatory hops from 23 GRNs reach a large part of the SEZ, so the counts, not the membership, carry the information; entries whose sugar → intermediate count is in single digits are weak paths. Sign comes from the connectivity table's `Excitatory x Connectivity` column (Excitatory × Connectivity, one sign per presynaptic neuron, the same column `sim/network.py` turns into weights), not from the annotation's neurotransmitter prediction; the two can disagree, as in row 2 (CB0207: +1 in the connectivity table, `glutamate` in the annotation), and that is the only such disagreement among the 20 rows.
