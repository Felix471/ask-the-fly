...
def apply_strings(entries: list[dict], check: bool, allow_new: bool = False,
    ...
    fly_lines = ROOT / 'copy' / 'fly_lines.json'
    if fly_lines.exists():
        payload = json.loads(fly_lines.read_text(encoding='utf-8'))
        (ROOT / 'site' / 'fly_lines.js').write_text(
            '// GENERATED by scripts/import_copy.py from copy/fly_lines.json; do not edit.\n'
            + 'export const FLY_LINES = ' + json.dumps(payload, ensure_ascii=False, indent=2) + ';\n', encoding='utf-8')
